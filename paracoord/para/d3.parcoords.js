...
function single_curve(d, ctx) {
	...
	for (var i = 1; i < cps.length; i += 3) {
		...
		//console.log(cps[i+2].e(1));
	}
	//console.log();
};

function single_curve_withoutNULL(d, j,ctx) {
	//console.log(xscale(d));
	var centroids = compute_centroids(d);
	var cps = compute_control_points(centroids);
	//ctx.strokeStyle = d3.functor(__.color)(d, j);
	ctx.beginPath();
	//if(d[__.dimensions[0]]==)
	var p=__.dimensions[0];
	var q=__.dimensions[1];
	var k=1;
	ctx.moveTo(cps[0].e(1), cps[0].e(2));
	var i;
	for (i = 1; i < cps.length-3; i += 3) {
		/*if (__.showControlPoints) {
			for (var j = 0; j < 3; j++) {
				ctx.fillRect(cps[i+j].e(1), cps[i+j].e(2), 2, 2);
			}
		...
		//if()
		ctx.bezierCurveTo(cps[i].e(1), cps[i].e(2), cps[i+1].e(1), cps[i+1].e(2), cps[i+2].e(1), cps[i+2].e(2));
		if(cps[i+2].e(1)==position(q)){
			//�ж���ɫ
			if(d[p]==nullValues[p]||d[q]==nullValues[q]){
			...
			ctx.beginPath();
			ctx.moveTo(cps[i+2].e(1), cps[i+2].e(2));
...
function color_path_withoutNULL(d, i, ctx) {
	/*ctx.strokeStyle = d3.functor(__.color)(d, i);
	ctx.beginPath();*/
	if (__.bundleDimension === null || (__.bundlingStrength === 0 && __.smoothness == 0)) {
		single_path_withoutNULL(d, i,ctx);
	} else {
		single_curve_withoutNULL(d, i,ctx);
	}
	//ctx.stroke();
};
...
function single_path_withoutNULL(d, j,ctx) {
	for(var i=0;i<__.dimensions.length-1;i++)
	{
			var p=__.dimensions[i];
			var q=__.dimensions[i+1];
			ctx.beginPath();
			
			ctx.moveTo(position(p), yscale[p](d[p]));
			ctx.lineTo(position(q), yscale[q](d[q]));
			if(d[p]==nullValues[p]||d[q]==nullValues[q])
				ctx.strokeStyle = 'rgba(255,255,255,0)';
			else
				ctx.strokeStyle= d3.functor(__.color)(d, j);
...
function path_foreground(d, i) {
	if(showNULL)
		...
		return color_path_withoutNULL(d, i, ctx.foreground);
};
...
function path_highlight(d, i) {
	if(showNULL)
		...
		return color_path_withoutNULL(d, i, ctx.highlight);
    ...
    .call(d3.behavior.drag()
      ...
      .on("dragend", function(d, i) {
        ...
        if (i !== j) {
          ...
          parent.insertBefore(this, parent.childNodes[j + 1])
        }
